...
export default function VendorCanadaPage() {
  ...
  const [fullName, setFullName] = useState(user?.full_name || 'Toronto Diaspora Importers Ltd');
  const [email, setEmail] = useState(user?.email || 'toronto.imports@diasporalink.ca');
  const [avatarUrl, setAvatarUrl] = useState(user?.avatar_url || '');
  const [phone, setPhone] = useState(user?.phone_number || '+1 416 555 0192');
  const [momoNumber, setMomoNumber] = useState(user?.momo_number || 'Stripe CAD Payout');
  ...
  const handleSavePreferences = async (e: React.FormEvent) => {
    e.preventDefault();
    const targetUserId = user?.id || 'usr_vca_1';
    ...
    toast.success('Seller preferences and profile saved.');
  };
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    const created = await dbService.createProduct({
      vendor_id: user?.id || 'usr_vca_1',
      vendor_name: user?.full_name || 'Toronto Diaspora Importers Ltd',
      title,
      description,
      category,
      price_cad: priceCad,
    ...
    resetForm();
    toast.success('Vendor product added to catalog.');
  };
  ...
  const menuItems = [
    ...
    { id: 'finances', label: 'Finances & Revenue', icon: <CreditCard size={18} /> },
    { id: 'analytics', label: 'Analytics & Most Liked', icon: <TrendingUp size={18} /> },
    { id: 'kyc', label: 'Canada Importer KYC', icon: <Building2 size={18} /> },
  ...
  return (
    <DashboardLayout
      ...
      menuItems={menuItems}
      title={user?.full_name || 'Toronto Diaspora Importers Ltd'}
      subtitle="Canada Importer / Vendor Portal • CRA BN-884920194 • YYZ Pearson Airport Hub"
    >
      {/* TAB 1: OVERVIEW */}
      {activeTab === 'overview' && (
        <div className="space-y-8">
          ...
          <div className="bg-black text-white p-8 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-xl">
            ...
            >
              <Plus size={16} className="inline mr-1.5" /> Add New Vendor Product
            </button>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-bold text-black font-retro-heading">Import Sales Performance Visualisation</h3>
            <VendorSalesChart role="vendor_canada" />
          </div>
        </div>
      )}
                  ...
                  {orders.length > 0 ? (
                    orders.map((o) => (
                      <tr key={o.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="p-4 font-mono text-xs font-bold text-black">{o.awb_number || 'AWB-YYZ-99201'}</td>
                        <td className="p-4">
      ...
      {activeTab === 'catalog' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h2 className="text-2xl font-bold text-black font-retro-heading">Manage Canadian Vendor Products</h2>
              <p className="text-xs text-gray-500 font-medium">Add, edit pricing, or toggle stock availability for your products.</p>
            </div>
            <button
              onClick={() => {
                resetForm();
            ...
            >
              <Plus size={16} className="inline mr-1.5" /> Add Product
            </button>
          </div>
      ...
      {activeTab === 'finances' && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-black font-retro-heading">Finances & Revenue Statements</h2>
          <div className="bg-white p-6 rounded-2xl border border-gray-200 space-y-3 text-xs font-medium">
            <div className="flex justify-between py-2 border-b">
              ...
              <span className="font-bold text-black">$34,800.00</span>
            </div>
            <div className="flex justify-between py-2 text-sm font-black text-black">
              <span>Net Settled Profits:</span>
              <span>$30,600.00 CAD</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-bold text-black font-retro-heading">Import Revenue Visualisation</h3>
            <VendorSalesChart role="vendor_canada" />
          </div>
                ...
                <tbody className="divide-y divide-gray-200 text-xs">
                  {products.map((prod, idx) => (
                    <tr key={prod.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-3.5 font-black text-red-600 font-mono">{240 - idx * 30} Likes</td>
                      <td className="p-3.5 font-black text-purple-600 font-mono">{115 - idx * 15} Saved</td>
                      <td className="p-3.5 font-bold font-mono text-black">{(2100 - idx * 250).toLocaleString()} Views</td>
      ...
      {/* TAB 6: CANADA IMPORTER KYC */}
      {activeTab === 'kyc' && (
        <div className="bg-white p-8 rounded-3xl border border-gray-200 max-w-2xl mx-auto space-y-6 shadow-xs">
          <h2 className="text-2xl font-bold text-black font-retro-heading">Canada Importer Business KYC Verification</h2>
          <p className="text-xs text-gray-500 font-medium">Official Canada Revenue Agency (CRA) Business Number & Importer Verification details.</p>
          <div className="space-y-4 text-xs font-medium">
            <div>
              <label className="font-bold text-black uppercase block mb-1">CRA Business Number (BN)</label>
              <input type="text" disabled value={user?.cra_business_number || 'BN-884920194'} className="w-full p-3 bg-gray-50 border rounded-xl font-mono font-bold text-sm text-black" />
            </div>
            <div>
              ...
              <div className="p-3.5 bg-gray-50 border rounded-xl font-bold text-black flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500" /> CRA Verified Canadian Importer
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSavePreferences} className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium">
                <div>
                  <label className="font-bold text-black uppercase block mb-1.5">Business / Importer Name</label>
                  <input
                    type="text"
                    required
                    value={fullName}
                ...
                <div>
                  <label className="font-bold text-black uppercase block mb-1.5">Payout Settlement Account</label>
                  <input
                    type="text"
                    value={momoNumber}
                    onChange={(e) => setMomoNumber(e.target.value)}
                    placeholder="Stripe CAD / Bank Wire"
                    className="w-full p-3 bg-gray-50 border border-gray-300 rounded-xl font-bold text-sm text-black focus:outline-none focus:border-black"
                  />
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-8 space-y-5 shadow-2xl relative border border-gray-200 my-8">
            ...
            <div className="space-y-1">
              <h3 className="text-2xl font-black text-[#111111] font-retro-heading uppercase flex items-center gap-2">
                <ShoppingBag size={24} /> {editingProduct ? 'Edit Product Details' : 'Add Vendor Product'}
              </h3>
              <p className="text-xs text-gray-500 font-medium">
                {editingProduct ? 'Update product pricing, title, image, description, or stock.' : 'List a new item for sale in your store.'}
              </p>
            </div>
            ...
            <form onSubmit={editingProduct ? handleUpdateProduct : handleCreateProduct} className="space-y-4 text-xs font-medium">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Specialty Import Product"
                  value={title}
